...
import json
import os
from .debug_utils import debugger   # Importar el módulo de depuración
...
def handle_game_controls(event, game, das_left_pressed, das_right_pressed, das_down_pressed, 
    ...
    if event.type == pygame.KEYDOWN:
        ...
        if not paused:
            ...
            # Rotación antihoraria (tecla Z)
            # No procesamos la rotación aquí para evitar duplicación
            # La rotación con Z se maneja directamente en game.py
            elif is_key_action(event, "rotate_inv", keybindings):
                debugger.debug("Z KEY PRESSED - Processing Z key rotation (handled in game.py)")
                pass
    ...
    # Comprobar estado de teclas mantenidas para movimientos continuos
    keys = pygame.key.get_pressed()
    
    # Verificar si alguna de las teclas de movimiento izquierda está presionada
    move_left_keys = []
    for binding in keybindings["keyboard"].get("move_left", []):
    ...
    # Verificar si alguna de las teclas de movimiento derecha está presionada
    move_right_keys = []
    for binding in keybindings["keyboard"].get("move_right", []):
        key_code = key_string_to_pygame_key(binding["key"])
        if key_code is not None:
    ...
    if not paused:
        ...
        if check_gamepad_action("move_left", keybindings) and not das_left_pressed:
            ...
            das_left_pressed = check_gamepad_action("move_left", keybindings) or das_left_pressed
        ...
        if check_gamepad_action("move_right", keybindings) and not das_right_pressed:
            ...
            das_right_pressed = check_gamepad_action("move_right", keybindings) or das_right_pressed
            
        if check_gamepad_action("rotate", keybindings):
            if game.rotate():
                sfx_rotate.play()
        
        if check_gamepad_action("rotate_inv", keybindings):
            # No procesamos la rotación aquí para evitar duplicación
            # La rotación con gamepad se maneja directamente en game.py
            debugger.debug("GAMEPAD ROTATE_INV - Processing rotation (handled in game.py)")
            pass
        ...
        if check_gamepad_action("soft_drop", keybindings) and not das_down_pressed:
            ...
            das_down_pressed = check_gamepad_action("soft_drop", keybindings) or das_down_pressed
