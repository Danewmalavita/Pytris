...
def start_game(screen, settings):
    ...
    if audio_available:
        try:
            ...
            sfx_move = pygame.mixer.Sound("assets/sounds/sfx/move.wav")  # Movimiento de pieza
            sfx_rotate = pygame.mixer.Sound("assets/sounds/sfx/rotate.wav")  # Rotación de pieza
            sfx_hard_drop = pygame.mixer.Sound("assets/sounds/sfx/harddrop.wav")  # Aterrizaje con espacio (hard drop)
            sfx_soft_drop = pygame.mixer.Sound("assets/sounds/sfx/softdrop.wav")  # Aterrizaje sin espacio (soft drop)
            sfx_landing = pygame.mixer.Sound("assets/sounds/sfx/landing.wav")  # Sonido al aterrizar la pieza
            sfx_single = pygame.mixer.Sound("assets/sounds/sfx/single.wav")  # Sonido al hacer una línea
            sfx_tetris = pygame.mixer.Sound("assets/sounds/sfx/tetris.wav")  # Sonido al hacer una linea de 4 (tetris)
            sfx_hold = pygame.mixer.Sound("assets/sounds/sfx/hold.wav")  # Sonido al hacer hold de una pieza
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if not paused:
                ...
                # Importar el módulo de controles unificado para verificar teclas
                from .controls import is_key_action, load_keybindings
                keybindings = load_keybindings()

                # Comprobar eventos de rotación inversa con teclas y gamepad
                if event.type == pygame.KEYDOWN and is_key_action(event, "rotate_inv", keybindings) and not paused and not game.game_over:
                    debugger.debug("Z KEY PRESSED - Handling rotation through control module")
                    if game.rotate_inv():
                        sfx_rotate.play()
                        
                if check_gamepad_action("rotate_inv") and not paused and not game.game_over:
                    debugger.debug("GAMEPAD ROTATE_INV - Handling rotation through gamepad")
                    if game.rotate_inv():
                        sfx_rotate.play()
