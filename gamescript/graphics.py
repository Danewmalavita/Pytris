...
class TetrisRenderer:
    ...
    def __init__(self, screen, block_size=30):
        ...
        # Cargar marcos del juego
        self.board_frame = pygame.image.load("assets/img/boardframe.png")
        self.hold_frame = pygame.image.load("assets/img/holdframe.png")
        self.next_frame = pygame.image.load("assets/img/nextframe.png")
    ...
    def draw_next_piece(self, game, x, y, box_width, box_height):
    ...
    def draw_game_info(self, game, next_piece_x, next_piece_y):
        ...
        # Marco y posición para próxima pieza
        frame_x = next_piece_x - 20  # Offset del borde del frame
        frame_y = next_piece_y - 20
        scaled_next_frame = pygame.transform.scale(self.next_frame, (frame_width, frame_height))
        ...
        score_y = next_piece_y + box_height + 60  # Ajustado para el nuevo tamaño
        draw_text(self.screen, f"Puntuación: {game.score}", 24, WHITE, next_piece_x + box_width // 2, score_y)
        draw_text(self.screen, f"Nivel: {game.level}", 24, WHITE, next_piece_x + box_width // 2, score_y + 30)
        draw_text(self.screen, f"Líneas: {game.lines_cleared}", 24, WHITE, next_piece_x + box_width // 2, score_y + 60)
        
        # Dibujar controles
        controls_y = score_y + 120
        draw_text(self.screen, "Controles:", 20, WHITE, next_piece_x + box_width // 2, controls_y)
        draw_text(self.screen, "←/A: Mover izda", 18, GRAY, next_piece_x + box_width // 2, controls_y + 25)
        draw_text(self.screen, "→/D: Mover dcha", 18, GRAY, next_piece_x + box_width // 2, controls_y + 45)
        draw_text(self.screen, "↑/X: Rotar dcha", 18, GRAY, next_piece_x + box_width // 2, controls_y + 65)
        draw_text(self.screen, "Z: Rotar izda ", 18, GRAY, next_piece_x + box_width // 2, controls_y + 85)
        draw_text(self.screen, "↓/S: Caída rápida", 18, GRAY, next_piece_x + box_width // 2, controls_y + 105)
        draw_text(self.screen, "Espacio: Soltar", 18, GRAY, next_piece_x + box_width // 2, controls_y + 125)
        draw_text(self.screen, "ESC: Pausar", 18, GRAY, next_piece_x + box_width // 2, controls_y + 145)
